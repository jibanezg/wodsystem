...
export class MortalWizard extends FormApplication {
  ...
  _initializeWizardData() {
    return {
      ...
      // Step 4: Advantages (Mortal baseline - only backgrounds, virtues, willpower)
      advantages: {
        backgrounds: [],
        virtues: this._initializeVirtues(), // Virtues for Mortal and Demon
        willpower: this.config.advantages?.willpower?.starting || 1
      },
   ...
   * For Mortal, uses original order (not M20 order)
   */
  _initializeAbilityValues() {
    ...
    // Initialize each category with abilities (for Mortal/Demon - no specific order needed)
    // Mage/Technocrat override this method to use M20 order
    for (const category of this.config.abilities.categories) {
      const categoryAbilities = actorAbilities[category] || {};
      
      // Iterate through all abilities in the actor's data
      for (const abilityName in categoryAbilities) {
        const rawValue = categoryAbilities[abilityName];
        if (rawValue !== undefined) {
          const numValue = Number(rawValue);
          // Only use if it's a valid number within reasonable bounds (0-10)
  ...
  /**
   * Initialize virtues for Mortal and Demon
   */
  _initializeVirtues() {
    const virtues = {};
    const startingValue = this.config.advantages?.virtues?.starting || 1;
    
    // Get virtues from actor's system data if available
    const actorVirtues = this.actor.system?.advantages?.virtues || {};
    
    // Initialize virtues from config (Conscience, Self-Control, Courage)
    if (this.config.advantages?.virtues?.available) {
      for (const virtueName of this.config.advantages.virtues.available) {
        // For character creation, always start at 1 (the config starting value)
        // Only use actor value if it's different from template default (3) - meaning it was actually set
        // This prevents new characters from inheriting template defaults
        const actorValue = actorVirtues[virtueName];
        if (actorValue !== undefined && actorValue !== 3) {
          // Actor has a non-default value, use it
          virtues[virtueName] = actorValue;
        } else {
          // Use wizard config starting value (1) for new characters
          virtues[virtueName] = startingValue;
        }
      }
    }
  ...
  _loadProgress() {
    ...
    // Load existing actor data into wizardData if not already set from saved progress
    // This ensures that if the actor already has data (e.g., from a previous wizard completion),
    // it will be loaded into the wizard
    // BUT: If saved progress exists, respect it (even if empty) - don't override with actor data
    if (!hasSavedProgress) {
      // Only load from actor if there's NO saved progress
      if (!this.wizardData.concept.name && this.actor.system?.identity?.name) {
        this.wizardData.concept.name = this.actor.system.identity.name;
      }
      if (!this.wizardData.concept.concept && this.actor.system?.identity?.concept) {
        this.wizardData.concept.concept = this.actor.system.identity.concept || "";
      }
      if (!this.wizardData.concept.nature && this.actor.system?.identity?.nature) {
        this.wizardData.concept.nature = this.actor.system.identity.nature;
      }
      if (!this.wizardData.concept.demeanor && this.actor.system?.identity?.demeanor) {
        this.wizardData.concept.demeanor = this.actor.system.identity.demeanor;
      }
      
      // Virtues (Mortal and Demon)
      if (this.config.advantages?.virtues) {
        // Only load from actor if wizardData doesn't already have virtues initialized
        // This ensures new characters start at 1, not whatever the actor template has
        if (this.actor.system?.advantages?.virtues && Object.keys(this.wizardData.advantages.virtues || {}).length === 0) {
          const actorVirtues = this.actor.system.advantages.virtues;
          for (const virtueName of this.config.advantages.virtues.available || []) {
            if (actorVirtues[virtueName] !== undefined) {
              this.wizardData.advantages.virtues[virtueName] = actorVirtues[virtueName];
            }
          }
        }
      }
      ...
      // Backgrounds
      // Only load backgrounds from actor if they have a value > 0 (don't load empty/default backgrounds)
      // AND only if there's no saved progress (to respect user's choice to remove all backgrounds)
      if (this.actor.system?.miscellaneous?.backgrounds && Array.isArray(this.actor.system.miscellaneous.backgrounds)) {
        if (!this.wizardData.advantages.backgrounds || this.wizardData.advantages.backgrounds.length === 0) {
          // Filter out backgrounds with no name or value of 0
          const validBackgrounds = this.actor.system.miscellaneous.backgrounds
            .filter(bg => bg.name && bg.name.trim() !== "" && bg.value > 0)
            .map(bg => ({
          ...
          if (validBackgrounds.length > 0) {
            this.wizardData.advantages.backgrounds = validBackgrounds;
          }
        }
      }
    } else {
      // If saved progress exists, ensure backgrounds is an array (even if empty)
      // This respects the user's choice to remove all backgrounds
      if (!Array.isArray(this.wizardData.advantages.backgrounds)) {
        this.wizardData.advantages.backgrounds = [];
      }
    }
    
    // Merits & Flaws
    if (this.actor.system?.miscellaneous?.merits && Array.isArray(this.actor.system.miscellaneous.merits)) {
      if (!this.wizardData.meritsFlaws.merits || this.wizardData.meritsFlaws.merits.length === 0) {
        this.wizardData.meritsFlaws.merits = this.actor.system.miscellaneous.merits.map(m => ({
          name: m.name || "",
          value: m.value || 0
        }));
      }
    ...
    if (this.actor.system?.miscellaneous?.flaws && Array.isArray(this.actor.system.miscellaneous.flaws)) {
      if (!this.wizardData.meritsFlaws.flaws || this.wizardData.meritsFlaws.flaws.length === 0) {
        this.wizardData.meritsFlaws.flaws = this.actor.system.miscellaneous.flaws.map(f => ({
          name: f.name || "",
          value: f.value || 0
        }));
      }
    ...
    // Load attributes and abilities for all creature types (already handled in _initializeAttributeValues and _initializeAbilityValues)
    // But ensure they're loaded if not already set from saved progress
    if (!this.wizardData.attributes.values || Object.keys(this.wizardData.attributes.values).length === 0) {
      this.wizardData.attributes.values = this._initializeAttributeValues();
    }
    if (!this.wizardData.abilities.values || Object.keys(this.wizardData.abilities.values).length === 0) {
      this.wizardData.abilities.values = this._initializeAbilityValues();
    }
  }
  ...
  async getData() {
    ...
    // Calculate willpower from Courage virtue for Mortal and Demon
    let calculatedWillpower = null;
    if (this.config.advantages?.virtues && this.wizardData.advantages.virtues) {
      calculatedWillpower = this.wizardData.advantages.virtues["Courage"] || this.config.advantages.virtues.starting || 1;
    }
    ...
    return {
      ...
      calculatedWillpower: calculatedWillpower,
  ...
  _activateConceptListeners(html) {
    // Restore all field values from wizardData after render
    // This ensures saved progress is properly displayed for both dropdowns and text inputs
    setTimeout(() => {
      // Restore select dropdowns
      html.find('select[name^="concept."]').each((i, select) => {
        const $select = $(select);
        const fieldName = $select.attr('name').split('.')[1];
        const savedValue = this.wizardData.concept[fieldName];
        
        if (savedValue) {
          // Try to set the value directly
          $select.val(savedValue);
          
          // Verify it was set (in case of type mismatch, whitespace, or case differences)
          const currentValue = $select.val();
          if (currentValue !== savedValue) {
            // Try case-insensitive and trimmed match
            const options = $select.find('option');
            let matched = false;
            const savedValueNormalized = String(savedValue).trim().toLowerCase();
            
            options.each((j, option) => {
              const optionValue = $(option).val();
              if (optionValue) {
                const optionValueNormalized = String(optionValue).trim().toLowerCase();
                if (optionValueNormalized === savedValueNormalized) {
      ...
      html.find('input[type="text"][name^="concept."], textarea[name^="concept."]').each((i, input) => {
        const $input = $(input);
        const fieldName = $input.attr('name').split('.')[1];
        const savedValue = this.wizardData.concept[fieldName];
        
        if (savedValue !== undefined && savedValue !== null && savedValue !== '') {
          const currentValue = $input.val();
          if (currentValue !== String(savedValue)) {
  ...
  _activateAttributesListeners(html) {
    ...
    // Randomize attributes button
    html.find('.randomize-attributes-btn').click(async (event) => {
      event.preventDefault();
      await this._randomizeAttributes();
    });
  ...
  _activateAbilitiesListeners(html) {
    ...
    // Randomize abilities button (reuse the same class name for styling)
    html.find('.step-abilities .randomize-attributes-btn').click(async (event) => {
      event.preventDefault();
  ...
  _activateAdvantagesListeners(html) {
    ...
    html.find('.background-select').on('change', async (event) => {
      ...
      if (this.wizardData.advantages.backgrounds[index]) {
        ...
        if (value && service && service.initialized) {
          const background = service.getBackgroundByName(value, this.actorType);
          if (background) {
    ...
    html.find('.step-advantages .background-reference-btn').click((event) => {
      ...
      if (backgroundName && service && service.initialized) {
        const background = service.getBackgroundByName(backgroundName, this.actorType);
    ...
    // Virtue increase/decrease (Mortal and Demon) - use sphere classes but filter by data-virtue
    html.find('.sphere-increase[data-virtue]').click(async (event) => {
      const virtue = event.currentTarget.dataset.virtue;
      await this._modifyVirtue(virtue, 1);
    });

    html.find('.sphere-decrease[data-virtue]').click(async (event) => {
      const virtue = event.currentTarget.dataset.virtue;
      await this._modifyVirtue(virtue, -1);
    });
  }
   ...
   * Modify virtue value
   */
  async _modifyVirtue(virtue, delta) {
    if (!this.wizardData.advantages.virtues[virtue]) {
      this.wizardData.advantages.virtues[virtue] = this.config.advantages.virtues.starting || 1;
    }
    
    const current = this.wizardData.advantages.virtues[virtue];
    const startingValue = this.config.advantages.virtues.starting || 1;
    const maxPoints = this.config.advantages.virtues.points;
    
    // Calculate what the new value would be
    const proposedValue = current + delta;
    
    // Enforce min/max per virtue (1-5)
    const newValue = Math.max(1, Math.min(5, proposedValue));
    
    // If increasing, check if we have enough points available
    if (delta > 0 && maxPoints !== undefined) {
      // Calculate current total points spent
      const currentTotal = Object.values(this.wizardData.advantages.virtues).reduce((sum, val) => {
        const numVal = Number(val) || startingValue;
        return sum + Math.max(0, numVal - startingValue);
      }, 0);
      
      // Calculate what the new total would be
      const pointsForThisVirtue = Math.max(0, newValue - startingValue);
      const otherVirtuesPoints = Object.entries(this.wizardData.advantages.virtues)
        .filter(([name]) => name !== virtue)
        .reduce((sum, [, val]) => {
          const numVal = Number(val) || startingValue;
          return sum + Math.max(0, numVal - startingValue);
        }, 0);
      
      const newTotal = otherVirtuesPoints + pointsForThisVirtue;
      
      // Prevent exceeding the limit
      if (newTotal > maxPoints) {
        ui.notifications.warn(game.i18n.format('WODSYSTEM.Wizard.TooManyVirtuePoints', { 
          spent: newTotal, 
          allowed: maxPoints 
        }) || `Too many virtue points spent (${newTotal}/${maxPoints})`);
        return; // Don't update the value
      }
    }
    
    this.wizardData.advantages.virtues[virtue] = newValue;
    ...
    // Find the specific virtue item (using sphere-item class to match template structure)
    const increaseBtn = html.find(`.sphere-increase[data-virtue="${virtue}"]`);
    const virtueItem = increaseBtn.closest('.sphere-item');
    
    if (virtueItem.length) {
      // Update the value display (using sphere-value class)
      virtueItem.find('.sphere-value').text(newValue);
      
      // Update the dots using the helper (max = 5, using sphere-dots class)
      const dotsDiv = virtueItem.find('.sphere-dots');
      const dotsHtml = Handlebars.helpers.renderDots(newValue, 5);
      dotsDiv.html(dotsHtml.toString());
      
      // Update button states for this specific virtue (using sphere classes)
      virtueItem.find('.sphere-decrease').prop('disabled', newValue === 1);
      
      // Check if increase should be disabled (either at max 5, or would exceed point limit)
      const maxPoints = this.config.advantages.virtues.points;
      let shouldDisableIncrease = newValue >= 5;
      
      if (maxPoints !== undefined && !shouldDisableIncrease) {
        // Calculate if increasing further would exceed the limit
        const startingValue = this.config.advantages.virtues.starting || 1;
        const currentTotal = Object.values(this.wizardData.advantages.virtues).reduce((sum, val) => {
          const numVal = Number(val) || startingValue;
          return sum + Math.max(0, numVal - startingValue);
        ...
        // If we're already at the limit, disable increase
        if (currentTotal >= maxPoints) {
          shouldDisableIncrease = true;
        }
      }
      
      virtueItem.find('.sphere-increase').prop('disabled', shouldDisableIncrease);
      
      // Also update all other virtue buttons in case the total changed
      if (maxPoints !== undefined) {
        ...
        html.find('.sphere-item[data-virtue]').each((i, item) => {
          const $item = $(item);
          const virtueName = $item.data('virtue');
          const virtueValue = this.wizardData.advantages.virtues[virtueName] || startingValue;
          const $increaseBtn = $item.find('.sphere-increase');
          
          // Disable if at max 5 OR if total is at limit and this virtue would increase it
          if (virtueValue >= 5 || (currentTotal >= maxPoints && virtueValue < 5)) {
    ...
    // Update willpower display (equals Courage virtue for Mortal and Demon)
    const courageValue = this.wizardData.advantages.virtues["Courage"] || this.config.advantages.virtues.starting || 1;
    const willpowerSection = html.find('.advantage-section:has(h4:contains("Willpower"))');
    if (willpowerSection.length) {
      willpowerSection.find('.willpower-info strong').text(courageValue);
    }
  ...
  _activateMeritsFlawsListeners(html) {
    ...
    html.find('.wizard-merit-name, .wizard-flaw-name').on('input', (event) => {
      ...
      if (service && service.initialized && query) {
        const reference = service.getByName(query, category, this.actorType);
        if (reference) {
    ...
    html.find('.wizard-merit-name, .wizard-flaw-name').each((idx, element) => {
      ...
      if (name && service && service.initialized) {
        const reference = service.getByName(name, category, this.actorType);
        if (reference) {
          $button.addClass('has-reference');
          $button.data('reference', reference);
        } else {
  ...
  async _selectWizardAutocomplete(input, result, category, index) {
    ...
    if (service && service.initialized) {
      const reference = service.getByName(result.name, category, this.actorType);
  ...
  async _postReferenceToChat(reference) {
    const service = game.wod?.referenceDataService;
    if (!service) return;
    const html = await foundry.applications.handlebars.renderTemplate('systems/wodsystem/templates/chat/reference-card.html', { reference });
  ...
  async _postBackgroundToChat(background) {
    const html = await foundry.applications.handlebars.renderTemplate('systems/wodsystem/templates/chat/background-reference-card.html', { background });
  ...
  _updateFreebiesButtonStates(html) {
    ...
    html.find('.freebie-decrease').each((i, btn) => {
      ...
      } else if (btnType === 'virtue') {
        const virtueName = btnTarget;
  ...
  /**
   * Randomize attributes: randomly assign priorities and distribute points
   */
  async _randomizeAttributes() {
    const categories = [...this.config.attributes.categories]; // physical, social, mental
    const priorities = ['primary', 'secondary', 'tertiary'];
    
    // Step 1: Randomly shuffle categories and assign to priorities
    const shuffledCategories = categories.sort(() => Math.random() - 0.5);
    this.wizardData.attributes.prioritySelection = {
      primary: shuffledCategories[0],
      secondary: shuffledCategories[1],
      tertiary: shuffledCategories[2]
    };
    
    // Step 2: Reset all attributes to 1 (starting value)
    for (const category of categories) {
      for (const attr in this.wizardData.attributes.values[category]) {
        this.wizardData.attributes.values[category][attr] = 1;
      }
    }
    
    // Step 3: Calculate available points for each priority
    const priorityPoints = {
      primary: this.config.attributes.priorities.primary,
      secondary: this.config.attributes.priorities.secondary,
      tertiary: this.config.attributes.priorities.tertiary
    };
    
    // Step 4: Distribute points randomly for each category
    for (const priority of priorities) {
      const category = this.wizardData.attributes.prioritySelection[priority];
      const pointsToDistribute = priorityPoints[priority];
      const attributes = Object.keys(this.wizardData.attributes.values[category]);
      
      // Create an array of attribute names with their current values (all start at 1)
      const attrArray = attributes.map(attr => ({ name: attr, value: 1 }));
      
      // Distribute points randomly
      let remainingPoints = pointsToDistribute;
      while (remainingPoints > 0) {
        // Pick a random attribute
        const randomIndex = Math.floor(Math.random() * attrArray.length);
        const attr = attrArray[randomIndex];
        
        // Increase it if it's not at max (5)
        if (attr.value < 5) {
          attr.value++;
          remainingPoints--;
        } else {
          // If this attribute is at max, try another one
          // Check if all attributes are at max
          if (attrArray.every(a => a.value >= 5)) {
            break; // Can't distribute more points
          }
        }
      }
      ...
      for (const attr of attrArray) {
        this.wizardData.attributes.values[category][attr.name] = attr.value;
      }
    }
  ...
  /**
   * Randomize abilities: randomly assign priorities and distribute points
   */
  async _randomizeAbilities() {
    const categories = [...this.config.abilities.categories]; // talents, skills, knowledges
    const priorities = ['primary', 'secondary', 'tertiary'];
    const maxAtCreation = this.config.abilities.maxAtCreation || 3;
    const startingValue = this.config.abilities.starting || 0;
    
    // Step 1: Randomly shuffle categories and assign to priorities
    const shuffledCategories = categories.sort(() => Math.random() - 0.5);
    this.wizardData.abilities.prioritySelection = {
      primary: shuffledCategories[0],
      secondary: shuffledCategories[1],
      tertiary: shuffledCategories[2]
    };
    
    // Step 2: Reset all abilities to starting value (usually 0)
    for (const category of categories) {
      for (const ability in this.wizardData.abilities.values[category]) {
        this.wizardData.abilities.values[category][ability] = startingValue;
      }
    }
    
    // Step 3: Calculate available points for each priority
    const priorityPoints = {
      primary: this.config.abilities.priorities.primary,
      secondary: this.config.abilities.priorities.secondary,
      tertiary: this.config.abilities.priorities.tertiary
    };
    
    // Step 4: Distribute points randomly for each category
    for (const priority of priorities) {
      const category = this.wizardData.abilities.prioritySelection[priority];
      const pointsToDistribute = priorityPoints[priority];
      const abilities = Object.keys(this.wizardData.abilities.values[category]);
      
      // Create an array of ability names with their current values (all start at startingValue)
      const abilityArray = abilities.map(ability => ({ name: ability, value: startingValue }));
      
      // Distribute points randomly
      let remainingPoints = pointsToDistribute;
      while (remainingPoints > 0) {
        // Pick a random ability
        const randomIndex = Math.floor(Math.random() * abilityArray.length);
        const ability = abilityArray[randomIndex];
        
        // Increase it if it's not at max
        if (ability.value < maxAtCreation) {
          ability.value++;
          remainingPoints--;
        } else {
          // If this ability is at max, try another one
          // Check if all abilities are at max
          if (abilityArray.every(a => a.value >= maxAtCreation)) {
            break; // Can't distribute more points
          }
        }
      }
      ...
      for (const ability of abilityArray) {
        this.wizardData.abilities.values[category][ability.name] = ability.value;
      }
    }
  ...
  async _addBackground() {
    ...
    // Get backgrounds list from service
    const service = game.wod?.referenceDataService;
  ...
  async _spendFreebie(type, target, delta, bgName = null) {
    ...
    switch(type) {
      ...
      case 'virtue':
        const virtueName = target;
        if (!this.wizardData.advantages.virtues) {
          this.wizardData.advantages.virtues = {};
        }
        const virtueStarting = this.config.advantages?.virtues?.starting || 1;
        const virtueCurrent = this.wizardData.advantages.virtues[virtueName] || virtueStarting;
        const virtueBaseline = baselines?.virtues?.[virtueName] || virtueStarting;
        const virtueMax = 5; // Virtues max is 5
        const virtueNew = Math.max(virtueBaseline, Math.min(virtueMax, virtueCurrent + delta));
        if (virtueNew !== virtueCurrent) {
          this.wizardData.advantages.virtues[virtueName] = virtueNew;
          valueChanged = true;
          newValue = virtueNew;
        }
        break;
    ...
    if (freebieItem.length) {
      ...
      // Virtue has similar structure to abilities
      const virtueItem = html.find(`.freebie-item:has(button[data-type="virtue"][data-target="${target}"])`);
      if (virtueItem.length) {
        virtueItem.find('.item-current').text(`${i18n('WODSYSTEM.Wizard.Current')}: ${newValue}`);
        const baselineValue = this.wizardData.freebies.baselines?.virtues?.[target] || this.config.advantages?.virtues?.starting || 1;
        virtueItem.find('.freebie-decrease').prop('disabled', newValue <= baselineValue);
        virtueItem.find('.freebie-increase').prop('disabled', newValue >= 5);
      }
    }
  ...
  async _onFinish(event) {
    ...
    try {
      ...
      console.error('MortalWizard | Error finalizing character:', error);
  ...
  async _applyToActor() {
    ...
    // Step 4: Advantages - Virtues (Mortal and Demon)
    if (this.config.advantages?.virtues) {
      const virtues = {};
      for (const virtueName of this.config.advantages.virtues.available || []) {
    ...
    // Willpower calculation - For Mortal and Demon, willpower = Courage virtue
    const courageValue = this.wizardData.advantages.virtues?.["Courage"] || this.config.advantages.virtues.starting || 1;
    let totalWillpower = courageValue;
