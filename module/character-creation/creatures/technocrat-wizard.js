/**
 * Technocrat Character Creation Wizard
 * Extends MortalWizard with Technocrat-specific functionality:
 * - Spheres
 * - Enlightenment (Arete)
 * - Convention, Amalgam, Eidolon
 */

import { MortalWizard } from '../base/mortal-wizard.js';
import { i18n } from '../../../module/helpers/i18n.js';

export class TechnocratWizard extends MortalWizard {
  /**
   * Initialize wizard data structure (Technocrat-specific additions)
   */
  _initializeWizardData() {
    const baseData = super._initializeWizardData();
    
    // Add Technocrat-specific concept fields
    baseData.concept.convention = "";
    baseData.concept.amalgam = "";
    baseData.concept.eidolon = "";
    
    // Add Technocrat-specific advantages
  ...
  _initializeSpheres() {
    ...
    // Initialize spheres for Technocrat
  ...
  /**
   * Initialize ability values from actor data (Technocrat uses M20 order)
   */
  ...
  /**
   * Load saved progress from actor flags (Technocrat-specific additions)
   */
  _loadProgress() {
    super._loadProgress();
    ...
    // Technocrat-specific concept fields
    if (!this.wizardData.concept.convention && this.actor.system?.identity?.convention) {
      this.wizardData.concept.convention = this.actor.system.identity.convention;
    }
    if (!this.wizardData.concept.amalgam && this.actor.system?.identity?.amalgam) {
      this.wizardData.concept.amalgam = this.actor.system.identity.amalgam;
    }
    if (!this.wizardData.concept.eidolon && this.actor.system?.identity?.eidolon) {
      this.wizardData.concept.eidolon = this.actor.system.identity.eidolon;
    }
  }

  /**
   * Get data for template rendering (Technocrat-specific additions)
   */
  async getData() {
  ...
  /**
   * Step: Concept listeners (Technocrat-specific additions)
   */
  _activateConceptListeners(html) {
    super._activateConceptListeners(html);
    
    // Technocrat-specific concept fields (convention, amalgam, eidolon) are handled by base class
    // No additional listeners needed beyond what base class provides
  }

  /**
   * Step: Advantages listeners (Technocrat-specific additions)
   */
  ...
  /**
   * Update freebie button states (Technocrat-specific: adds sphere handling)
   */
  ...
  /**
   * Recalculate freebie points remaining from scratch (Technocrat-specific additions)
   */
  ...
  /**
   * Spend freebie points (Technocrat-specific: adds sphere handling)
   */
  ...
  /**
   * Apply wizard data to actor (Technocrat-specific additions)
   */
  async _applyToActor() {
    // Build base updateData (replicate parent logic but add Technocrat-specific fields)
    const updateData = {};
    
    // Step 1: Concept (add Technocrat-specific fields)
    ...
    // Willpower calculation - For Technocrat, use base + freebies
